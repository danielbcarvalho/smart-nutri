...
const MealList: React.FC<MealListProps> = ({
  ...
  const renderFoodItem = (mealFood: MealFood) => {
    ...
    const substitutes =
      mealFood.substitutes
        ?.map((sub) => {
          const subFood = foodDb.find((f) => f.id === sub.substituteFoodId);
          if (!subFood) return null;

          // Encontrar a medida caseira correta baseada na unidade do substituto
          const subMcIndex =
            subFood.mc?.findIndex((mc) => mc.nome_mc === sub.substituteUnit) ??
            0;
          const subPeso = Number(subFood.mc?.[subMcIndex]?.peso) || 1;
          const quantidadeGramas = Number(sub.substituteAmount) * subPeso;
