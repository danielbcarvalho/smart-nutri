...
export const PhotoUpload: React.FC<ExtendedPhotoUploadProps> = ({
  ...
  useEffect(() => {
    ...
    if (initialPhotoUrl !== state.previewUrl && !state.isUploading) {
      console.log(
        `PhotoUpload (${type}): initialPhotoUrl changed. Updating preview to:`,
        initialPhotoUrl
      );
      setState((prev) => ({
        ...prev,
  ...
  const processFile = async (file: File) => {
    ...
    if (onUploadStart) {
      try {
        ...
        if (!canProceed) {
          console.log(
            `PhotoUpload (${type}): Upload cancelled by onUploadStart`
          );
          setState((prev) => ({
            ...prev,
            isUploading: false,
        ...
        console.error(
          `PhotoUpload (${type}): Error during onUploadStart:`,
          err
        );
        const error =
    ...
    try {
      ...
      console.log(`PhotoUpload (${type}): Upload success. Photo:`, photo);
      // Pass the AssessmentPhoto object up
      onUploadComplete(photo); // Pass the correct type
    } catch (err) {
      console.error(`PhotoUpload (${type}): Upload failed:`, err);
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      if (state.isUploading) {
        console.log(`PhotoUpload (${type}): Drop ignored, upload in progress.`);
  ...
  const handleRemoveClick = async (e: React.MouseEvent) => {
    ...
    console.log(`PhotoUpload (${type}): Remove clicked`);
    ...
    if (onRemove) {
      try {
        await onRemove(type);
        console.log(`PhotoUpload (${type}): Parent onRemove handler finished.`);
      } catch (error) {
        console.error(
          `PhotoUpload (${type}): Error in parent onRemove handler:`,
          error
        );
  ...
  const handleEditClick = (e: React.MouseEvent) => {
    ...
    if (inputElement) {
      console.log(
        `PhotoUpload (${type}): Edit clicked, triggering file input.`
      );
      inputElement.click();
    } else {
      console.warn(
        `PhotoUpload (${type}): Could not find input element to trigger edit.`
      );
    }
    setIsHovering(false);
  };
          ...
          {!state.isUploading && effectivePreviewUrl && showPreview ? (
            <Box sx={{ width: "100%", height: "100%", position: "relative" }}>
              <img
                ...
                onError={(e) => {
                  console.warn(
                    `PhotoUpload (${type}): Failed to load image ${effectivePreviewUrl}`
                  );
